Authors : Ingrid, Lucas and Raulph, Peltrop


1. Type in "gcc hw4compiler.c" (this will create a file name a.out) in terminal window
2. Then type in "./a.out input.txt" in terminal window or a corresponding text file.

#Note: The quotation marks ("") are not included.
#Note2: You may have an output file that is a .exe extension which also is accepted.